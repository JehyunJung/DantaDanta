...
**1시간봉** 데이터 기준으로 추세 전환 시점에 진입한다. 세 가지 지표가 모두 맞아야 매수한다.

- **KRX**: 봇 시작 시 yfinance에서 60일치 시간봉을 즉시 적재하고, 이후 매 15분마다 KIS 분봉을 수집해 시간봉으로 리샘플링하여 DB에 누적한다.
- **해외**: 일봉 데이터 사용 (KIS 해외 분봉 미지원).

**사용 지표 설명**

- **EMA (지수이동평균)**: 최근 주가의 평균선. 단기선(3시간)이 장기선(10시간)을 아래에서 위로 뚫고 올라가면 상승 추세 전환 신호 → **골든크로스**. 반대로 위에서 아래로 내려가면 하락 신호 → **데드크로스**.
...
| 국장 스크리너 | 평일 08:30, 08:50 (장전) + 09:00 - 15:00 매 30분 | KRX 종목 캐시 갱신 |
| 미장 스크리너 | 평일 22:30, 22:50 (장전) + 23:00 - 05:00 매 30분 | 해외 종목 캐시 갱신 |
| KRX 시간봉 수집 | 평일 09:05 - 15:50, 매 15분 | yfinance 초기 적재 후 KIS 분봉으로 갱신 |

> 스크리너는 백그라운드에서 미리 계산해 캐시에 저장하므로 웹 대시보드에서 즉시 조회된다 (캐시 유효 35분).
...
# KIS 실거래 앱키 — 해외 시세 조회 전용 (필수)
# KIS 모의투자 서버는 해외 주식 시세 API를 지원하지 않는다.
# 해외 종목(NASD/NYSE 등) 스크리닝·매매 시 실거래 앱키로 실서버에 직접 요청한다.
# KIS API 포털에서 실거래용 앱을 별도 등록 후 발급.
...
- KIS 모의투자 서버는 해외 주식 시세 API 미지원 — `KIS_REAL_APP_KEY`를 설정하면 해외 시세 요청만 실거래 서버로 자동 우회
- 해외 시세 API는 초당 1건 제한 (EGW00201) — 자동 딜레이 적용
