...
일봉 데이터 기준으로 추세 전환 시점에 진입한다.

| 조건 | 내용 |
|------|------|
| 매수 | EMA(3)가 EMA(10)을 상향 돌파 (골든크로스) + RSI 30-65 구간 + MACD 히스토그램 양전환 |
| 매도 | EMA(3)가 EMA(10)을 하향 돌파 (데드크로스) 또는 RSI > 75 |
| 손절 | 설정값 (기본 -3%) |
| 익절 | 설정값 (기본 +7%) |

MACD 히스토그램이 음→양으로 전환되는 시점이면 신뢰도 0.9, 히스토그램이 양수이면 0.7로 분류한다.

### 뉴스 감성 필터 (매수 전 적용)

매수 신호 발생 시 Claude API로 해당 종목 관련 뉴스를 분석해 감성 점수(-1.0 - 1.0)를 산출한다. 설정값(기본 -0.3) 미만이면 매수를 보류한다. 웹 설정 페이지 또는 텔레그램에서 활성화/비활성화 가능.

### 스캘핑 전략 (옵션, 60초 루프)

1분봉 기준 단기 과매도 반등을 노린다. 설정에서 활성화해야 동작한다.

| 조건 | 내용 |
|------|------|
| 매수 | 종가 < BB(20) 하단 AND RSI < 30 |
| 매도 | 종가 > BB 중심선 OR RSI > 65 OR 최대 보유 시간 초과 |
| 손절 | 설정값 (기본 -0.5%) |
| 익절 | 설정값 (기본 +1.0%) |
